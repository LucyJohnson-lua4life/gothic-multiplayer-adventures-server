...
function VLK_7006_Weapontrader()
	local npc = CreateNPC(GetNewNPCName("Gerald (2H Weapons)"));
	...
	--Items:
	EquipArmor(npc,"ItNm_Panzermantel2");
	--EquipMeleeWeapon(npc,"ItMw_1h_Bau_Mace");

	
	local npcarr = {};
	npcarr.id = npc;
	npcarr.daily_routine = TA_VLK_7006;
...
function TA_VLK_7006(_playerid)
	
	if(TA_FUNC(_playerid, 0, 0, 24, 0))then
		AI_ClearStates(_playerid);
		AI_SETWALKTYPE(_playerid, 0);--Let him walk!
		AI_GOTOFP(_playerid, "FP_STAND_NW_TAVERNE_IN_RANGERMEETING_05");
		AI_ALIGNTOFP(_playerid, "FP_STAND_NW_TAVERNE_IN_RANGERMEETING_05");
