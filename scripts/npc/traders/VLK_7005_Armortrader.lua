...
function VLK_7005_Armortrader()
	local npc = CreateNPC(GetNewNPCName("Bryan (Armors)"));
	
	SetPlayerAdditionalVisual(npc,"Hum_Body_Naked0",1, "Hum_Head_FatBald", 72);
	...
	local npcarr = {};
	npcarr.id = npc;
	npcarr.daily_routine = TA_VLK_7005;
...
function TA_VLK_7005(_playerid)
	
	if(TA_FUNC(_playerid, 0, 0, 24, 0))then
		AI_ClearStates(_playerid);
		AI_SETWALKTYPE(_playerid, 0);--Let him walk!
		AI_GOTOFP(_playerid, "FP_STAND_NW_TAVERNE_IN_RANGERMEETING_04");
		AI_ALIGNTOFP(_playerid, "FP_STAND_NW_TAVERNE_IN_RANGERMEETING_04");
